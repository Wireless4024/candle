...
use crate::op::{Op, ReduceOp, UnaryOp};
use crate::{Result, Tensor, TensorId};
use ahash::*;
...
impl GradStore {
    ...
    pub fn combine(&mut self, other: &Self) -> Result<()> {
        use std::collections::hash_map::Entry;
        for (tid, og) in other.0.iter() {
            match self.0.entry(*tid) {
                Entry::Vacant(e) => {
                    // No grad yet for this id → copy it in.
                    e.insert(og.clone());
                }
                Entry::Occupied(e) => {
                    // Sum into existing grad. Align device/dtype first.
                    let new_val = {
                        let lhs = e.get(); // &Tensor already in self
                        // Move/cast rhs as needed to match lhs
                        let rhs_aligned = {
                            let rhs_dev = if lhs.device().same_device(og.device()) {
                                og.clone()
                            } else {
                                og.to_device(lhs.device())?
                            };
                            if lhs.dtype() == rhs_dev.dtype() {
                                rhs_dev
                            } else {
                                rhs_dev.to_dtype(lhs.dtype())?
                            }
                        };
                        // Add and return the summed tensor
                        (lhs + &rhs_aligned)?
                    };
                    *e.into_mut() = new_val;
                }
