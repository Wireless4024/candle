...
use crate::{Error, Storage, Tensor, WithDType};

// arg has been reduced to node via reduce_dims, expand it back to arg.
// This has to handle keepdims.
pub fn broadcast_back(arg: &Tensor, node: &Tensor, reduced_dims: &[usize]) -> crate::Result<Tensor> {
...
impl Tensor {
    ...
    /// # Params
    /// * `detach` - If true, detach the gradients from the graph. default would be true
    /// # Returns
    /// * `grads` - A map from the nodes to their gradients.
    /// # Errors
    /// * `Error::BackwardNotSupported` - If the backward operation is not supported.
    pub fn backward2(&self, detach: bool) -> crate::Result<GradStore> {
        ...
        for node in sorted_nodes.iter() {
            ...
            let grad = if detach { grad.detach() } else { grad };
    ...
    /// unsafe interface to mutate raw buffer of the tensor directly
    /// # Safety
    /// - This function is unsafe because it bypass rwlock and ignore immutability of tensor.
    /// - This only works on CPU tensor.
    ///
    pub unsafe fn get_raw_buffer<D: WithDType>(&mut self) -> crate::Result<*mut [D]> {
        let mut storage = self.storage_mut();
        match &mut *storage {
            Storage::Cpu(storage) => {
                storage.as_slice_mut::<D>().map(|x| x as *mut [D])
            }
            _ => crate::bail!("Tensor is not on CPU"),
        }
    }

    /// get shape when treat this tensor as weight of linear layer.
    /// returns (in, out)
    pub fn get_linear_shape(&self) -> crate::Result<(usize, usize)> {
        let dims = self.shape().dims();
        if dims.len() != 2 {
            crate::bail!("Tensor is not 2D tensor");
        }
        Ok((dims[1], dims[0]))
    }
}