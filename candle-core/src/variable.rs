...
#[derive(Clone, Debug)]
pub struct Var(Tensor, crate::tweaks::VariableKind);
...
impl Var {
    ...
    pub fn rand<S: Into<Shape>, T: crate::FloatDType>(lo: T, up: T, s: S, device: &Device) -> Result<Self> {
        let inner = Tensor::rand_impl(lo, up, s, device, true)?;
        Ok(Self::new_internal(inner))
    }

    pub fn randn<S: Into<Shape>, T: crate::FloatDType>(mean: T, std: T, s: S, device: &Device) -> Result<Self> {
    ...
    pub fn from_vec<S: Into<Shape>, D: crate::WithDType>(data: Vec<D>, shape: S, device: &Device) -> Result<Self> {
        let inner = Tensor::from_vec_impl(data, shape, device, true)?;
        Ok(Self::new_internal(inner))
    }

    pub fn from_slice<S: Into<Shape>, D: crate::WithDType>(array: &[D], shape: S, device: &Device) -> Result<Self> {
...
impl Var {
    fn new_internal(inner: Tensor) -> Self {
        Self(inner, crate::tweaks::VariableKind::default())
    }

    pub fn optimizer_hint(&self) -> crate::tweaks::VariableKind {
        self.1
    }

    pub fn set_optimizer_hint(&mut self, hint: crate::tweaks::VariableKind) {
        self.1 = hint;
    }
}
